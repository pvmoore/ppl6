module ppl._3_resolve.ResolveIdentifier;
...
private:
    void findLocalOrGlobal(Identifier n) {
        ...
        auto res = module_.findIdentifierTarget.find(n.name, n);
    ...
    void findStructMember(Identifier n) {
        ...
        switch(n.name) {
            case "length":
                if(prevType.isArray) {
                    ...
                    folder.fold(dot, LiteralNumber.makeConst(len.to!string, TYPE_INT));
                    return;
            ...
            case "subtype":
                ...
                /// for arrays only
                if(prevType.isArray) {
                    folder.fold(dot, TypeExpr.make(prevType.getArrayType.subtype));
                    return;
                } else if(prevType.isEnum) {
        ...
        if(!prevType.isStruct() && !prevType.isClass() && !prevType.isTuple() && !prevType.isEnum()) {
            module_.addError(prev, "Left of identifier %s must be a struct, class or enum type not a %s (prev=%s)".format(n.name, prevType, prev), true);
            return;
        }
        ...
        if(e) {
            ...
            folder.fold(dot, ExpressionRef.make(em));
            return;
        }
        ...
        if(var) {
            ...
            // Add a null check
            if(module_.config.enableNullChecks) {
                if(prevType.isPtr && prev.isIdentifier) {
                    auto id = prev.getIdentifier();

                    // Assume 'this' will never be null
                    if("this" != id.name) {
