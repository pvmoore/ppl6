module ppl._3_resolve.ResolveBuiltinFunc;
        ...
        switch(n.name) {
            case "sizeOf":
                if(n.numExprs() > 0) {
                    ...
                    folder.fold(n, LiteralNumber.makeConst(size.to!string, TYPE_INT));
                }
                break;
            case "alignOf":
                if(n.numExprs() > 0) {
                    int align_ = n.exprs()[0].getType().alignment();
                    folder.fold(n, LiteralNumber.makeConst(align_.to!string, TYPE_INT));
                }
                break;
            case "initOf":
                if(n.numExprs() > 0) {
            ...
            case "isPointer":
                if(n.numExprs() > 0) {
                    ...
                    folder.fold(n, LiteralNumber.makeConst(r ? TRUE_STR : FALSE_STR, TYPE_BOOL));
                }
                break;
            case "pointerDepth":
                if(n.numExprs() > 0) {
                    int r = n.exprTypes()[0].getPtrDepth();
                    folder.fold(n, LiteralNumber.makeConst(r.to!string, TYPE_INT));
                }
                break;
            case "isValue":
                if(n.numExprs() > 0) {
            ...
            case "isInteger":
                if(n.numExprs() > 0) {
                    ...
                    folder.fold(n, LiteralNumber.makeConst(b ? TRUE_STR : FALSE_STR, TYPE_BOOL));
                }
                break;
            case "isReal":
                if(n.numExprs() > 0) {
            ...
            case "isFunction":
                if(n.numExprs() > 0) {
                    ...
                    folder.fold(n, LiteralNumber.makeConst(isFunc ? TRUE_STR : FALSE_STR, TYPE_BOOL));
                }
                break;
            case "isFunctionPtr":
                if(n.numExprs > 0) {
            ...
            case "ctUnreachable":
                ...
                break;
            // @deprecated - use unsigned integer types instead when ready
            case "ult":
            case "ulte":
            case "ugt":
            case "ugte":
