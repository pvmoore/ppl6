...
enum Operator : Op {
    ...
    UDIV     = Op(8, 6, "unsigned /"),   // Not currently used?
    UMOD     = Op(9, 6, "unsigned %"),   // Not currently used?
    ...
    SHL      = Op(12, 7, "<<"),
    SHR      = Op(13, 7, ">>"),
    USHR     = Op(14, 7, ">>>"), // To be removed when unsigned integer types are added
    BIT_AND  = Op(15, 7, "&"),
    BIT_XOR  = Op(16, 7, "^"),
    BIT_OR   = Op(17, 7, "|"),
    ...
    ULT       = Op(22, 9, "unsigned <"),
    UGT       = Op(23, 9, "unsigned >"),
    ULTE      = Op(24, 9, "unsigned <="),
    UGTE      = Op(25, 9, "unsigned >="),
...
Operator parseOperator(Tokens t) {
    ...
    if(t.kind() == TT.RANGLE) {
        if(t.peek(1).kind == TT.RANGLE) {
            if(t.peek(2).kind == TT.RANGLE) {
                t.next(2);
    ...
    if(t.kind() == TT.LSQBRACKET) {
        if(t.peek(1).kind == TT.RSQBRACKET) {
            t.next();
            return Operator.INDEX;
        }
    }
    auto p = t.kind() in g_ttToOperator;
    if(p) return *p;
    switch(t.value()) {
