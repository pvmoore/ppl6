module ppl._5_check.CheckModule;
...
public:
            ...
            if(v.name) {
                ...
                if(ptr) {
                    ...
                    if(sameScope) {
                        ...
                        /// Check for shadowing
                        auto res = module_.findIdentifierTarget.find(v.name, v.previous());
                        if(res) {
    ...
    void visit(Variable n) {
        ...
        if(n.isStructVar()) {
            ...
            if(struct_.isPOD() && !n.isPublic) {
                module_.addError(n, "POD struct member variables must be public", true);
            }
            if(struct_.isVisibleToOtherModules() && n.isPublic) {
...
private:
    void recursiveVisit(ASTNode node) {
        //dd("check", typeid(node));
        node.visit!CheckModule(this);
        foreach(n; node.children) {
            recursiveVisit(n);
        }

        if(node.endPos == INVALID_POSITION) {
            assert(node.line()==-1 && node.column()==-1);
        }
    }
    ...
    void checkMemberFunctionAccess(ASTNode self, Function targetFunc) {
        /// Target is public
        if(targetFunc.isPublic) return;

        auto inSameModule = self.getModule() == targetFunc.getModule();

        Struct targetStruct = targetFunc.getStructOrClass();
        // Struct thisStruct     = self.getAncestor!Struct;
        // bool isExternalAccess = (thisStruct is null) || thisStruct != targetStruct;
    ...
    void checkForExposingPrivateType(ASTNode node, Type t, string msgPrefix = null) {
        ...
        if(struct_) {
            if(!struct_.isPublic) errorType = struct_;

        } else if(array) {
            checkForExposingPrivateType(node, array.subtype, msgPrefix);
        } else if(enum_) {
            if(!enum_.isPublic) errorType = enum_;
        } else if(tuple_) {
