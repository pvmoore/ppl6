module ppl._2_parse.ParseExpression;
...
private:
    void parseLHS(Tokens t, ASTNode parent) {
        ...
        if(parent.isDot() && t.kind()==TT.IDENTIFIER) {
        ...
        if(eot!=-1) {
            ...
            if(nextTok.kind==TT.LBRACKET && t.onSameLine(eot+1)) {
                /// type(
        ...
        switch(t.kind()) with(TT) {
            ...
            default:
                ...
                errorBadSyntax(module_, t, "Syntax error at %s".format(t.kind));
                return;
        }
    }
    void parseRHS(Tokens t, ASTNode parent) {
        ...
        while(true) {
            ...
            if("is"==t.value()) {
                ...
                case TT.EQUALS:
                    module_.addError(t, "Reassignment. Did you mean to write ':=' ?", true);
                    goto case TT.PLUS;
                case TT.PLUS:
                case TT.MINUS:
                case TT.DIV:
                ...
                case TT.ASTERISK:
                    ...
                    break;
                case TT.AMPERSAND:
                    if(t.peek(1).kind==TT.AMPERSAND) errorBadSyntax(module_, t, "Did you mean 'and'");
                    parent = attachAndRead(t, parent, parseBinary(t));
                    break;
                case TT.PIPE:
                    if(t.peek(1).kind==TT.PIPE) errorBadSyntax(module_, t, "Did you mean 'or'");
    ...
    void parseParenthesis(Tokens t, ASTNode parent) {
        ...
        if(t.kind() == TT.RBRACKET) errorBadSyntax(module_, t, "Empty parenthesis");
    ...
    void parseUnary(Tokens t, ASTNode parent) {
        ...
        } else if(t.kind() == TT.TILDE) {
            u.op = Operator.BIT_NOT;
        } else if(t.kind() == TT.MINUS) {
            u.op = Operator.NEG;
    ...
    void parseConstructor(Tokens t, ASTNode parent) {
        ...
        /// Build a call to the default constructor new(this):
        ///
        /// Constructor
        ///     Call 'new'
        ///
        ...
        while(!t.isKind(TT.RBRACKET)) {
            ...
            if(t.peek(1).kind == TT.COLON) {
                ...
                if(call.paramNames.length>0) {
                    module_.addError(con, "Mixing named and un-named constructor arguments", true);
                } else if(parens.numChildren() > 0) {
                    module_.addError(con, "Too many unnamed constructor arguments", true);
                }
