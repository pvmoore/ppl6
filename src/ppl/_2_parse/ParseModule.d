module ppl._2_parse.ParseModule;
...
public:
    ...
    void appendTokensFromTemplate(ASTNode afterNode, Token[] tokens) {
        ...
        if(afterNode.isFunction) {
            t.setAccessPublic(afterNode.as!Function.isPublic);
        } else {
            assert(afterNode.id==NodeID.STRUCT);
            t.setAccessPublic(afterNode.as!Struct.isPublic);
        }
        ...
        templateParseComplete = false;
        module_.resolver.setTokensModified();
    }
private:
    void tokenise() {
        watch.start();
    ...
    void collectTypesAndFunctions() {
        ...
        bool isFuncDecl() {
            ...
            if(t.isKeyword("fn") && t.peek(1).kind != TT.LBRACKET) return true;
        ...
        while(t.hasNext()) {
            ...
            } else if(t.kind() == TT.LCURLY) {
                /// Skip {} block
                auto eob = t.findEndOfBlock(t.kind());
                if(eob==-1) {
                    module_.addError(t, "Couldn't find matching bracket }", false);
                    break;
                }
                t.next(eob);
            } else if(t.kind() == TT.LSQBRACKET) {
                /// Skip [] block
                auto eob = t.findEndOfBlock(t.kind());
                if(eob==-1) {
                    module_.addError(t, "Couldn't find matching bracket ]", false);
    ...
    void addRealProgramEntry(Function main) {
        ...
        Type gc = module_.findType.findType("GC", main);
