...
struct Token {
    ...
    static Token make(TT kind, string value, int length, Position start, Position end) {
        return Token(kind, value, length, start, end, null);
    }

    string toString() {
        string t   = kind==TT.IDENTIFIER ? "'"~value~"'" : "%s".format(kind);
...
string toSimpleString(Token t) {
    return t.kind==TT.IDENTIFIER ? t.value  :
           t.kind==TT.NUMBER ? t.value      : t.kind.toString();
}
