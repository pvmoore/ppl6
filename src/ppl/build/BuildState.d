...
protected:
    ...
    Module[string] modules;     // key = canonicalName
...
public:
    ...
    void dumpStats(void delegate(string) receiver = null) {
        ...
        receiver("IR generation time ..... %.2f ms".format(allModules().map!(it=>it.generator.getElapsedNanos()).sum() * 1e-6));
...
protected:
    void parseResolveFold() {
        logState("[✓] parseResolveFold");

        enum MAX_LOOPS      = 100;
        auto prevUnresolved = new Set!int;
        bool stalemate      = false;

        auto parseResolveFold = new ParseResolveFoldPass();

        for(int loop=1; !hasErrors() && loop<MAX_LOOPS; loop++) {
            ...
            parseResolveFold.runSinglePass(allModules(), stalemate);

            bool nodesModified = parseResolveFold.isModified();
            auto unresolved    = parseResolveFold.getUnresolved();
