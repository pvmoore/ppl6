...
///
    ...
    override string toString() {
        ...
        string kind = isExtern ? "extern" :
                      isImport ? "import" :
                      isGlobal ? "global" : "member";
        string s;
        if(isTemplateBlueprint()) {
            s ~= "<" ~ blueprint.paramNames.join(",") ~ "> ";
        }
        return "Function '%s'%s [kind=%s] %s%s [refs=%s,%s]".format(name, s, kind, mod, getAccessString(isPublic), numRefs, numExternalRefs);
    }
}
