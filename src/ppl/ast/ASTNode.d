...
/// Override these
    ...
    /**
     *  Collect a list of all descendents that are of type T.
     */
    void selectDescendents(T)(ref T[] array) {
        auto t = this.as!T;
        if(t) array ~= t;
    ...
    /**
     *  Collect all T nodes where filter returns true, recursively.
     */
    void recursiveCollect(T)(ref T[] array, bool delegate(T n) filter) {
        T t = this.as!T;
        if(t && filter(t)) array ~= t;
        foreach(n; children) {
            n.recursiveCollect!T(array, filter);
        }
    }
    /**
     * Collect all Targets from Identifier and Call nodes which are descendents of node.
     */
    void collectTargets(ref Target[] targets) {
